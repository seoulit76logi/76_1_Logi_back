...
public class CompInfoServiceImpl implements CompInfoService {
    ...
    public PaymentStatusTO getPaymentStatus(String customerCode){return compInfoMapper.selectPaymentStatus(customerCode);};
    ...
    public void updateDealDetailInfo(DealDetailTO to){
        compInfoMapper.updateDealDetailInfo(to);
    }
    public void batchCustomer(List<CustomerTO> customerList) {
        for (CustomerTO to : customerList) {
			...
			switch (status) {
				...
				case "delete":
					codeMapper.deleteCustomer(to.getCustomerName());
					break;
			}
		}
